...
export default function CancelFlowModal({ isOpen, onClose, token }) {
  ...
  const handleSurveyNext = async (selectedReason, feedback) => {
    ...
    try {
      const res = await axios.post(`${(import.meta.env.PROD ? "" : "http://localhost:5001")}/api/subscription/cancel-reason`, {
  ...
  const handleAcceptOffer = async () => {
    setLoading(true);
    try {
      await axios.post(`${(import.meta.env.PROD ? "" : "http://localhost:5001")}/api/subscription/accept-offer`, {
  ...
  const handleConfirmCancel = async () => {
    setLoading(true);
    try {
      await axios.post(`${(import.meta.env.PROD ? "" : "http://localhost:5001")}/api/subscription/confirm-cancel`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setIsCancelled(true);
      setTimeout(() => {
