...
const fetchProperties = async (token) => {
  const { data } = await axios.get(`${(import.meta.env.PROD ? "" : "http://localhost:5001")}/api/properties`, {
...
const Properties = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (newProp) => axios.post(`${(import.meta.env.PROD ? "" : "http://localhost:5001")}/api/properties`, newProp, {
      headers: { Authorization: `Bearer ${token}` }
    }),
    onSuccess: () => {
