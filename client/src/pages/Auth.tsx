...
const Auth = () => {
  ...
  const googleClientId = (import.meta.env.VITE_GOOGLE_CLIENT_ID || '').replace(/['"]/g, '').trim();
  const isGoogleLoginConfigured = /^[0-9]+-[a-z0-9_-]+\.apps\.googleusercontent\.com$/i.test(googleClientId);
            ...
            <div className="mt-6">
              ...
              <div className="mt-6 flex flex-col gap-3 justify-center items-center">
                {isGoogleLoginConfigured ? (
                  ...
                  <div className="w-full rounded border border-amber-500/50 bg-amber-500/10 p-3 text-center text-xs text-amber-200">
                    Google ile giriş geçici olarak kullanılamıyor. E-posta ve şifrenizle giriş yapabilirsiniz.
                  </div>
                )}
              </div>
            </div>
